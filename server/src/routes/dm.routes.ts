...
export const dmRoutes = {
    "/dm/invite": {
        POST: withDebugLog(async (req) => {
            ...
            if (!checkTimeWindow(time, 60)) return Response.json({ status: "error", message: "签名时间不在允许范围" }, { status: 400 });
    ...
    "/dm/invites/:username": {
        GET: withDebugLog((req) => {
            ...
            if (!checkTimeWindow(timestamp, 60)) return Response.json({ status: "error", message: "签名时间不在允许范围" }, { status: 400 });
